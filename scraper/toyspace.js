...
export async function scrapeToyspace() {
  const results = [];
  try {
    const html = await fetchWithBrowser("https://toyspace.se/lego", 5000);
      ...
      try {
        ...
        for (const item of items) {
          ...
          results.push({
            ...
            name: item.name,
            store: "ToySpace",
            store_url: item.offers?.url ?? "https://toyspace.se",
            price_local: price,
            currency: "SEK",
    ...
    if (!results.length) {
      $("[class*='product'], article").each((_, el) => {
        ...
        results.push({
          ...
          store: "ToySpace",
          store_url: link ? (link.startsWith("http") ? link : `https://toyspace.se${link}`) : "https://toyspace.se",
          price_local: price,
          currency: "SEK",
    ...
    console.log(`[ToySpace] Scraped ${results.length} products`);
  } catch (e) {
    console.error("[ToySpace]", e.message);
  }
  return results;
}
